...
extension RuleSet: Mappable {
    ...
    public convenience init?(map: Map) {
        ...
        var rules: [Rule] = []
        //ww: if let parsedObject = Mapper<Rule>().mapArray(JSONArray: rulesJSON as! [[String : Any]]){
        let parsedObject = Mapper<Rule>().mapArray(JSONArray: rulesJSON as! [[String : Any]]) //ww:
...
extension Alamofire.DataRequest {
    ...
    public static func ObjectMapperArraySerializer<T: Mappable>(_ keyPath: String?) -> DataResponseSerializer<[T]> {
        return DataResponseSerializer { request, response, data, error in
            ...
            if (JSONToMap != nil) {
                //ww: if let parsedObject = Mapper<T>().mapArray(JSONArray: JSONToMap as! [[String : Any]]){
                let parsedObject = Mapper<T>().mapArray(JSONArray: JSONToMap as! [[String : Any]]) //ww:
                return .success(parsedObject)
                //}
            }
